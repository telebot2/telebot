do
local Dev =  135693512--put your id here(BOT OWNER ID)
...
local function setrank(msg, name, value) -- setrank function
  ...
  if hash then
	...
	return send_msg('chat#id'..msg.to.id, 'درجه ی کاربر ('..name..') به '..value..' تغییر داده شد ', ok_cb,  true)
  end
end
   ...
   Username = 'Nadarad'
    ...
    if not value then
	 if result.id == tonumber(Dev) then
	   text = text..': Modir Asli Robot ⭐⭐⭐⭐ (Executive Admin) \n\n'
	  elseif is_admin2(result.id) then
	   text = text..': Admin Robt ⭐⭐⭐ (Admin) \n\n'
	  elseif is_owner2(result.id, extra.chat2) then
	   text = text..': Modir Asli Gp ⭐⭐ (Owner) \n\n'
	  elseif is_momod2(result.id, extra.chat2) then
	    text = text..':Modir Gp ⭐ (Moderator) \n\n'
      else
	    text = text..':member 🎈 (Member) \n\n'
	 end
   else
   text = text..'Darage : '..value..'\n\n'
  end
  local uhash = 'user:'..result.id
  local user = redis:hgetall(uhash)
  local um_hash = 'msgs:'..result.id..':'..extra.chat2
  user_info_msgs = tonumber(redis:get(um_hash) or 0)
  text = text..'Tedad Pm dar Gp : '..user_info_msgs..'\n\n'
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'آیدی' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local text = 'First Name : '..(msg.from.first_name or 'Nadarad')..'\n'
   local text = text..'Last Name : '..(msg.from.last_name or 'Nadarad')..'\n'	
   local text = text..'User : '..Username..'\n'
   local text = text..'Id Gp : '..msg.from.id..'\n\n'
   local hash = 'rank:'..msg.to.id..':variables'
    ...
    text = text..'Darage : '..value..'\n\n'  
	 ...
	 user_info_msgs = tonumber(redis:get(um_hash) or 0)
	 text = text..'Tedade Pm dar Gp : '..user_info_msgs..'\n\n'
	 if msg.to.type == 'chat' then
	 text = text..'Group Name : '..msg.to.title..'\n'
     text = text..'Id Gp : '..msg.to.id
    end
	text = text..'\n\nTelewild Bot'
