...
--vardump(extra)
      ...
      local receiver = "chat#id"..chat_id
       if get_cmd == "kick" then
         if member_id == from_id then
             return send_large_msg(receiver, "شما قادر به کیک نیستید")
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            return send_large_msg(receiver, "شما قادر به کیک مدیران و ادمین ربات نیستید")
         end
      ...
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از تمام گروه ها آنبن شد')
    ...
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(matches[2], '^%d+$') then
        ...
        if not is_admin(msg) and is_momod2(matches[2], msg.to.id) then
          	return "شما قادر به بن مدیران و ادمین ربات نیستید"
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
          	return "شما قادر به بن کردن نیستید!"
        end
        local name = user_print_name(msg.from)
  ...
  if matches[1]:lower() == 'unban' then -- /unban 
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
        	...
        	local name = user_print_name(msg.from)
        	savelog(msg.to.id, name.." ["..msg.from.id.."] بن شد ".. matches[2])
...
if matches[1]:lower() == 'kick' then
	...
	if string.match(matches[2], '^%d+$') then
		...
		if not is_admin(msg) and is_momod2(matches[2], msg.to.id) then
			return "شما قادر به کیک کردن مدیران و ادمین ربات نیستید"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "شما قادر به کیک کردن نیستید!"
		end
      		local user_id = matches[2]
      		local chat_id = msg.to.id
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then 
        	...
        	return 'کاربر ['..user_id..' ] از همه ی گزوه ها آنبن شد'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'unbanall',
